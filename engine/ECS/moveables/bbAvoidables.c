#include "engine/ECS/moveables/bbAvoidables.h"POINTS_PER_TILE
...
bbFlag bbAvoidables_new(bbAvoidables** self, I32 squares_i, I32 squares_j)
{
    ...
    bbVPool_newBloated(&avoidables->pool, sizeof(bbAvoidable),10,1000, "bbAvoidable");
...
bbMilliCoords bbAvoidables_sumForces (bbMoveables* moveables, bbAvoidables* avoidables, bbMoveable* moveable)
{
    ...
    if (SC.i>=0 && SC.j>=0 && SC.i < avoidables->size.i && SC.j < avoidables->size.j)
    {
        ...
        {
            single = bbAvoidable_getForce(moveables, moveable,avoidable);
            total.i += single.i;
            total.j += single.j;
