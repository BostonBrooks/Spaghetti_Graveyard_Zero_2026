...
*/
///Delete Pool
bbFlag bbSystemPool_delete(void* pool);

///Clear Pool
bbFlag bbSystemPool_clear(void* pool);

///Allocate an element then return address and handle by reference
bbFlag bbSystemPool_allocImpl(void* pool, void** address, bbHandle* handle, char* file, int line);

///De-allocate an element
bbFlag bbSystemPool_free(void* pool, void* address);

///Lookup an element from handle
bbFlag bbSystemPool_lookup(void* pool, void** address, bbHandle handle);

///Lookup a handle from element
bbFlag bbSystemPool_reverseLookup(void* pool, void* address, bbHandle* handle);

///Print element meta-data
bbFlag bbSystemPool_printHeader(void* pool, void* address);

///return true if handles are equal. return different flag for system/index/generation mismatch
bbFlag bbSystemPool_handleIsEqual(void* NULL, bbHandle a, bbHandle b);
...
bbFlag bbSystemPool_allocFromHandle(void* pool, void** address, bbHandle handle, char* file, I32 line);
