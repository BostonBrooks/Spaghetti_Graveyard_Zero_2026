...
bbFlag bbViewport_init(bbViewport* viewport, I32 height, I32 width)
{
    ...
    bbRenderable_init(&VP->highlight, height, width);
    ...
    char fragShader[] = "\
        ...
        {\
            ...
            vec4 highlight  = texture2D(Highlight,  gl_TexCoord[0].xy);\
            vec4 mapicon    = texture2D(MapIcon,    gl_TexCoord[0].xy);\
        \
            vec4 mix1 = mix(background, ground,    ground.a);\
            vec4 mix2 = mix(mix1,       main,      main.a  );\
            vec4 mix3 = mix(mix2,       highlight, highlight.a / 2.0 );\
            vec4 mix4 = mix(mix3,       mapicon,   mapicon.a / 2.0  );\
\
            gl_FragColor =  mix4;\
            \
        }\
    ...
    sfShader_setTextureUniform(VP->shader,"Highlight", VP->highlight.texture);
...
bbFlag bbViewport_draw(sfRenderWindow* window, bbViewport* viewport){
    ...
    sfRenderTexture_display(viewport->highlight.renderTexture);
...
bbFlag bbViewport_clear(bbViewport* viewport){
    ...
    sfRenderTexture_clear(viewport->highlight.renderTexture, sfTransparent);
